...
const Article = ({ title, description, image, category, rating, location, timestamp, user, onPress, }) => {
    const { t } = useTranslation();
    const { colors, gradients, icons, sizes } = useTheme();
    // render card for Newest & Fashion
    if (category?.id !== 1) {
        return (React.createElement(TouchableWithoutFeedback, { onPress: onPress },
            React.createElement(Block, { card: true, padding: sizes.sm, marginTop: sizes.sm },
                ...
                description && (React.createElement(Text, { p: true, marginTop: sizes.s, marginLeft: sizes.xs, marginBottom: sizes.sm }, description)),
                ...
                (Boolean(location) || Boolean(rating)) && (React.createElement(Block, { row: true, align: "center" },
    ...
    return (React.createElement(TouchableWithoutFeedback, { onPress: onPress },
        React.createElement(Block, { card: true, white: true, padding: 0, marginTop: sizes.sm },
            React.createElement(Image, { background: true, resizeMode: "cover", radius: sizes.cardRadius, source: { uri: image } },
                React.createElement(Block, { color: colors.overlay, padding: sizes.padding },
                    ...
                    React.createElement(Text, { p: true, white: true }, description),
